/*******************************************
  GLOBALS
*********************************************/
const { log, errors, cozyClient } = require('cozy-konnector-libs'),
...
function retriveArticles() {
  return retrieveBookmarksList(1).then(() => {
    log(
      'info',
      'all bookmarks list downloaded, there are ' +
        CTXT.bookmarksList.length +
        ' articles'
    )
  ...
  return CTXT.requestFactory({ jar: true, cheerio: false, json: true })
    ...
    .then(data => {
      ...
      if (data.currentPage < data.totalPages) {
        ...
        // return filterAlreadyDownladedArticles() TODO
      }
    })
    .catch(error => {
...
function prepareBookmarksFileNames() {
  ...
  CTXT.bookmarksList.forEach(bookmark => {
    bookmark.filename =
      sanitizeFileName(
        moment(bookmark.publicationDate).format('YYYY-MM-DD') +
          ' - ' +
          bookmark.title
      ) + '.html'
  })
      ...
      return isAlreadyDownloaded(bookmark)
        ...
        .catch(error => {
          if (error.message === 'fileAlreadyHere') {
            log(
              'info',
              `useless to download https://www.lesechos.fr/x/x/-${bookmark.id}`
            )
          } else {
...
function retrieveArticle(bookmark) {
  log(
    'info',
    'fetch article from ' + `https://www.lesechos.fr/x/x/-${bookmark.id}`
  )
  return CTXT.requestFactory({
    jar: CTXT.cookieJar,
    cheerio: true,
    json: false
  })
...
async function saveArticle(article) {
  const fs = require('fs')
  fs.writeFileSync(article.filename, article.inlinedHtml)
  ...
  fileDoc = await cozyClient.files.create(article.inlinedHtml, {
    name: filename,
